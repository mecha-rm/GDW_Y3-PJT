...
public class TesterScript : MonoBehaviour
{
	...
	void Start()
    {
		RecordManager fs = new RecordManager();
		...
		if(true)
        {
			...
			byte[] data = RecordManager.SerializeObject(x);
			...
			x = (Vec3)RecordManager.DeserializeObject(data);
		...
		
		{
			...
			for(int i = 0; i < count; i++)
            {
				if(i == byteIndex) // byte storage
                {
					...
					Vec3 x2 = new Vec3();
					object obj = RecordManager.DeserializeObject(impData);
