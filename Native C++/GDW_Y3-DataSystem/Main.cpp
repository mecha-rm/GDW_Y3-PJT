...
int main()
{
    ...
    // string test 
    {
        ...
        // edit attempt 1
        int index = dm.GetDataRecordCount() - 1;
        char* charArrTemp = new char[] {"abc"};
        
        dm.ReplaceDataRecord(index, charArrTemp, 3); // edits record
        dm.ReplaceDataRecord(index, charArr, str.length());  // returns back to original
    ...
    // char array test (uses dynamic variable since static would be deleted upon leaving)
    {
        ...
        tempDr.data = new char[]{"abc"};
        tempDr.size = 3;

        dm.ReplaceDataRecord(index, tempDr); // edits record
        dm.ReplaceDataRecord(index, dr); // returns original record
        delete[] tempDr.data;
